...
class TbbApiRequestTest {
    ...
    fun registrationEndpointsSendAuthorizedTypedRequests() = runBlocking {
        ...
        api.addGuest("c1", UpsertGuestRequest("Aunt Vol", ShirtSize.AM, GuestAgeTier.AGE_9_PLUS, Gender.FEMALE))
        assertEquals("POST" to "/registration/c1/guests", methods.last() to requests.last())

        api.updateGuest("g1", UpsertGuestRequest("Aunt V.", ShirtSize.AL, GuestAgeTier.AGE_9_PLUS, Gender.FEMALE))
